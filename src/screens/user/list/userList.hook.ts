...
export const useUserList = (data: UserListHook) => {
  ...
  const handlePrefetchUser = async ({ _id: userId }: any) => {
    await queryClientInstance.prefetchQuery(
      ["user", userId],
      async () => {
        const { data = null } = (await api.get(`/user/load?_id=${userId}`)) || {};
        return data;
      },
  ...
  const deleteUser = useMutation(
    ...
    {
      onSuccess: () => {
        queryClientInstance.invalidateQueries(["users", data.page]);
        queryClientInstance.refetchQueries(["users", data.page]);
        router.reload();
      },
      onError: () => {
        showModal({
  ...
  const deleteSelectedAction = async () => {
    deleteUser.mutateAsync(users.filter((user: UserProps) => user.value));
  };
